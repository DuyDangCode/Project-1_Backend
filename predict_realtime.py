...
class VideoDescriptionRealTime(object):
    ...
    def greedy_search(self, loaded_array):
        ...
        states_value = self.inf_encoder_model.predict(
            loaded_array.reshape(-1, 80, 4096))
    ...
    def get_test_data(self, choosed_file):
        ...
        if os.path.exists(path):
            ...
            try:
                ...
                try:
                    ...
                    print("Err: Extract features fail.")
            except:
                print("Err: Model load fail.")

        return f, file_name

    def test(self, choosed_file):
        X_test, filename = self.get_test_data(choosed_file)
...
def predictRealtime(choosed_file="download_video.mp4"):
    video_to_text = VideoDescriptionRealTime(config)
    # while True:
    print('.........................\nGenerating Caption:\n')
    start = time.time()
    video_caption, file = video_to_text.test(choosed_file)
    end = time.time()
    ...
    shutil.rmtree(os.path.join(config.test_path, 'video'))
    os.makedirs(os.path.join(config.test_path, 'video'))
